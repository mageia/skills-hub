#!/usr/bin/env python3
from __future__ import annotations

import argparse
from pathlib import Path
from waninter_utils import (
    create_output_dir,
    decode_b64_to_file,
    download_file,
    extract_b64_image,
    extract_media_url,
    extract_task_id,
    fail,
    guess_extension_from_url,
    load_config,
    request_json,
    write_json,
)

ASPECT_TO_SIZE = {
    "1:1": "1024x1024",
    "16:9": "1536x864",
    "9:16": "864x1536",
    "4:3": "1365x1024",
    "3:4": "1024x1365",
}


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate an image with Waninter Creative")
    parser.add_argument("--prompt", required=True)
    parser.add_argument("--model")
    parser.add_argument("--size")
    parser.add_argument("--aspect-ratio")
    parser.add_argument("--n", type=int, default=1)
    parser.add_argument("--output", default="./outputs")
    parser.add_argument("--response-format")
    args = parser.parse_args()

    cfg = load_config()
    size = args.size or ASPECT_TO_SIZE.get(args.aspect_ratio or "", cfg.get("default_image_size", "1024x1024"))
    model = args.model or cfg["default_image_model"]
    response_format = args.response_format or cfg.get("image_response_format", "b64_json")
    out_dir = create_output_dir(args.output, "image-")

    payload = {
        "model": model,
        "prompt": args.prompt,
        "size": size,
        "n": args.n,
        "response_format": response_format,
    }
    data = request_json(cfg, "POST", cfg["image_generation_path"], payload)
    write_json(out_dir / "response.json", data)

    file_path = None
    media_url = extract_media_url(data)
    if media_url:
        ext = guess_extension_from_url(media_url, ".png")
        file_path = download_file(media_url, out_dir / f"image{ext}")
    else:
        b64 = extract_b64_image(data)
        if b64:
            file_path = decode_b64_to_file(b64, out_dir / "image.png")

    task_id = extract_task_id(data)
    result = {
        "type": "image",
        "status": "succeeded" if file_path or media_url or task_id else "unknown",
        "file": str(file_path) if file_path else None,
        "url": media_url,
        "task_id": task_id,
        "prompt": args.prompt,
        "model": model,
        "parameters": {"size": size, "aspect_ratio": args.aspect_ratio, "n": args.n},
        "output_dir": str(out_dir),
    }
    write_json(out_dir / "metadata.json", result)
    if not (file_path or media_url or task_id):
        fail("Could not find image URL, base64 image, or task id in provider response", output_dir=str(out_dir))
    print_json(result)


def print_json(data):
    import json
    print(json.dumps(data, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()
