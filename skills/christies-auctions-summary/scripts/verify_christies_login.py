...
def connect_agent_browser(cdp: str) -> None:
    completed = subprocess.run(['agent-browser', 'connect', resolve_cdp_target(cdp)], text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False)
...
def run_agent_browser(args: list[str]) -> subprocess.CompletedProcess[str]:
    return subprocess.run(['agent-browser', *args], text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False)
...
def classify_login_state(snapshot_text: str, body_text: str) -> tuple[bool, str]:
    ...
    if found_in and found_out:
        return False, f'conflicting markers: in={found_in}, out={found_out}'
    if found_in:
...
def main() -> None:
    ...
    parser.add_argument('--url', default='https://www.christies.com/en/results/')
    ...
    if not is_logged_in:
        raise SystemExit(f"Christie's login-enhanced precondition failed; {reason}")

    print(f"Christie's login appears valid for login-enhanced workflow ({reason})")
