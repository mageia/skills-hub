#!/usr/bin/env python3
"""Orchestrate login-enhanced Sotheby's auction summary collection."""
...
def normalize_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    formats = set(output.get("format") or ["markdown", "json"])
    ...
    return {
        ...
        "output_dir": expand_path(output.get("dir", "./outputs/sothebys-auctions-summary")),
    }
...
def main() -> None:
    ...
    login_cmd = [sys.executable, str(script_dir() / "verify_sothebys_login.py"), "--cdp-port", str(config["cdp"]["port"])]
    if config["cdp"].get("url"):
        login_cmd.extend(["--cdp-url", config["cdp"]["url"]])
    run(login_cmd)
    run([
        sys.executable,
        str(script_dir() / "fetch_sothebys_auctions.py"),
